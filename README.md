...
상명대학교 컴퓨터과학전공 부스에서 운영한 **축제 한정 매칭 서비스**.
사용자가 뽑기권을 소진하며 이성 회원의 프로필을 한 명씩 공개받고, 마음에 들면 노출된 인스타그램 ID로 직접 컨택하는 흐름.

축제 부스에서 현장 결제 → 8자리 PIN 발급 → 앱에서 코드 입력 → 뽑기권 충전 → 매칭의 전 과정을 처리한다.
...
- **이메일 OTP 가입** — 학번(`@sangmyung.kr`) 입력 → Supabase Auth Confirm Sign Up → 8자리 코드 확인 → 프로필 작성. 같은 브라우저가 아닌 곳에서 메일을 열어도 코드 입력만으로 가입 완료 가능.
- **뽑기 + 다시뽑기(reroll)** — 잔액 1장씩 차감하며 이성 프로필 카드 공개. 캐릭터 선호 필터 지원. 한 번 본 카드는 `seen_users`로 추적되어 재노출 안 됨.
- **현장 결제 → PIN 충전** — 운영자가 발급한 1회용/공유용 PIN을 사용자가 앱에서 입력해 뽑기권 충전. 공유 PIN은 모든 유저가 1번씩 사용 가능 (`redemption_code_uses`).
- **운영자 어드민** — `profiles.is_admin = true` 인 유저만 접근 가능한 PIN 발급/잔액 직접 부여 화면.
- **본 행사 게이트** — `EVENT_START_AT` 이전엔 가입/프로필 작성만 허용, 피드/충전은 차단 (proxy + API 양쪽에서 검증).
- **가입자 마일스톤 보상** — 누적 가입 인원 도달 시 전 회원에게 뽑기권 자동 적립 알림.
...
│   ├── auth/          # check-user, send-code (resend)
│   ├── cron/          # 미완성 가입자 정기 정리
│   ├── feed/          # next, reroll
│   ├── redeem/        # 코드 충전
│   ├── stats/         # 가입자 카운터
